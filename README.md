...
## Overview
The project implements a miniature payroll pipeline on ARM (Thumb) assembly. It loads attendance, overtime, and performance data into RAM, builds employee records, runs payroll calculations, sorts employees by net salary, produces department summaries, and emits a UART payslip demo.
...
- UART registers: `UART0_DR = 0x4000C000`, `UART0_FR = 0x4000C018` (`UART_TXFF_BIT = 0x20`).

## How to run
1. Open `Assignment-2/Assignment-2_Solution.s` in an ARM/Thumb-supporting toolchain (e.g., Keil uVision or an ARM Cortex-M simulator).
2. Build/assemble and load to a Cortex-M target or emulator; ensure RAM is mapped at the addresses above.
